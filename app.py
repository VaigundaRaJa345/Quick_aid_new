...
@login_required
def dashboard():
    details = UserDetails.query.filter_by(user_id=current_user.id).all()
...
@app.route('/view_details/<uid>')
@login_required
def view_details(uid):
    details = db.session.query(UserDetails).filter_by(uid=uid, user_id=current_user.id).first()
